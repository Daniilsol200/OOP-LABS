...
namespace EquipmentRegistryApp
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            // Добавляем обработчики события PreviewKeyDown для всех DataGrid'ов
            ...
            if (e.Key == Key.Delete && dgOrganizations.SelectedItems.Count > 0)
            {
                try
                {
                    ...
                    foreach (var org in selectedOrganizations)
                    {
                        if (org.OrgId != 0)
                        {
                            // Проверяем, используется ли организация в таблице оборудования
            ...
            if (e.Key == Key.Delete && dgTypes.SelectedItems.Count > 0)
            {
                try
                {
                    ...
                    foreach (var type in selectedTypes)
                    {
                        if (type.TypeId != 0)
                        {
                            // Проверяем, используется ли тип в таблице оборудования
